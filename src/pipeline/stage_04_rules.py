...
def run_stage_04_rules(df: pd.DataFrame, config: dict, run_dir: Path) -> pd.DataFrame:
    ...
    modeling_df = df[df['phlegm_dampness_label_flag'] == 1].copy() if 'phlegm_dampness_label_flag' in df.columns else df.copy()
    ...
    coverage_matrix = build_rule_coverage_matrix(modeling_df, rules, candidates)
    ...
    target = modeling_df['risk_tier'] == 'high'
    ...
    save_payload(
        {
            'modeling_population': 'phlegm_only',
            'modeling_sample_size': int(len(modeling_df)),
